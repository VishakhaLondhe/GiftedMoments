...
if (updatedField.contact || state.contact) {
  ...
  if (!state.contact || !contactRegex.test(state.contact)) {
    ...
    // Validate GST Number (assuming it should be a certain format, modify regex accordingly)
    // if (updatedField.gstNo || state.gstNo) {
    //   const gstRegex = /^[0-9]{15}$/;  // Example: Modify this based on the format you expect
    //   if (!state.gstNo || !gstRegex.test(state.gstNo)) {
    ...
    // }
      ...
      // Validate GST Number
//     if (updatedField.gstNo || state.gstNo) {
//   // Assuming GST number format: 15 characters, including numbers and uppercase letters
...
if (updatedField.gstNo || state.gstNo) {
  ...
  if (!trimmedGstNo || !gstRegex.test(trimmedGstNo)) {
    ...
    //   const regNoRegex = /^[A-Za-z0-9]{10}$/;  // Example format
